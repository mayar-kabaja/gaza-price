  ...
  if (isDesktop) {
    return (
      <section className="mb-4">
        ...
        {/* Price cards — 2-col grid on desktop */}
        <div className="grid grid-cols-3 gap-3">
          {showLoading ? (
            <div className="flex items-center justify-center py-6 col-span-3">
              <LoaderDots size="sm" />
            </div>
          ) : isError ? (
            <p className="text-mist text-xs py-4 col-span-3">تعذر تحميل الأسعار</p>
          ) : prices.length === 0 ? (
            <p className="text-mist text-xs py-4 col-span-3">لا توجد أسعار لهذا المنتج بعد</p>
          ) : (
            prices.map((price) => (

...
export function ListingCard({ listing, isSaved = false, onSaveToggle }: ListingCardProps) {
  ...
  return (
    ...
    <div
      ...
      className="bg-surface rounded-2xl mb-3 shadow-sm border border-border/60 overflow-hidden transition-all hover:shadow-md cursor-pointer active:scale-[0.99]"
    >
      <div className="flex items-stretch">
        {/* Image — larger, prominent */}
        ...
        <div className="flex-1 px-3 py-3 min-w-0 flex flex-col justify-between">
          ...
          <div className="flex items-start justify-between gap-1 mb-1">
            <div className="font-bold text-[13px] text-ink leading-snug flex-1 line-clamp-2">
              {listing.title}
            </div>
            <button
              onClick={handleSave}
              className="flex-shrink-0 p-0.5 -mt-0.5 text-mist transition-colors"
              aria-label="حفظ"
            >
          ...
          <div className="flex items-center gap-1.5 flex-wrap mb-2">
            <span className={cn("text-[10px] font-bold px-2 py-[2px] rounded-full", cond.cls)}>
              {cond.label}
            </span>
            {listing.area?.name_ar && (
              <span className="text-[10px] text-mist flex items-center gap-[2px]">
                <svg viewBox="0 0 24 24" className="w-[9px] h-[9px]" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round">
          ...
          {/* Bottom: price left, time right */}
          <div className="flex items-center justify-between">
            <span className="font-display font-black text-[17px] text-olive-deep leading-none" dir="ltr">
...
export function ListingCardSkeleton() {
  return (
    <div className="bg-surface rounded-2xl mb-3 border border-border/60">
      <div className="flex items-stretch">
        <div className="w-[130px] flex-shrink-0 p-2.5">

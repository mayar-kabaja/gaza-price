      ...
      >
        ...
        <div className="px-5 pt-3 pb-4 border-b border-[var(--d-border)]/60">
          <div className="flex items-start justify-between">
            <div>
              <div className="font-bold text-[15px] text-[var(--d-text)]">{order.customer_name} <span className="text-[var(--d-text-muted)]">#{order.order_number}</span></div>
              <a
                href={`https://wa.me/${order.customer_phone.replace(/[^0-9]/g, "")}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-1 text-[12px] text-[var(--d-text-muted)] hover:text-[#25D366] transition-colors mt-1"
                dir="ltr"
              >
          ...
          <div className="flex items-center justify-between mt-3 text-[11px] text-[var(--d-text-muted)]">
            <div className="flex items-center gap-1.5">
              <svg viewBox="0 0 24 24" className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
...
function OrderCard({ order, rejectId, setRejectId, rejectReason, setRejectReason, updateMutation, newOrderFlash, onSelect }: {
  ...
  return (
    ...
    >
      ...
      <div className={`px-4 pt-4 pb-3 border-b border-[var(--d-border)]/60 ${isDead ? "opacity-50" : ""}`}>
        <div className="flex items-start justify-between">
          <div className="min-w-0">
            <div className="font-semibold text-[13px] text-[var(--d-text)] truncate">{order.customer_name} <span className="text-[var(--d-text-muted)]">#{order.order_number}</span></div>
            <a
              href={`https://wa.me/${order.customer_phone.replace(/[^0-9]/g, "")}`}
              target="_blank"
              rel="noopener noreferrer"
              className="text-[10px] text-[var(--d-text-muted)] hover:text-[#25D366] transition-colors mt-0.5 block text-right"
              dir="ltr"
            >
              {order.customer_phone}
            </a>
      ...
      {/* ── Date & Time ── */}
      <div className={`flex items-center justify-between px-4 py-2 text-[10px] text-[var(--d-text-muted)] ${isDead ? "opacity-50" : ""}`}>
        <div className="flex items-center gap-1.5">
          {isToday(order.created_at) && (
      ...
      <div className={`flex-1 min-h-0 overflow-hidden ${isDead ? "opacity-40" : ""}`}>
        <div className="overflow-y-auto max-h-full px-4 pt-2 space-y-1">
          {order.items.map((item) => (
            <div key={item.id} className="flex items-center justify-between text-[11px] py-0.5">
              <span className="text-[var(--d-text)] truncate flex-1 ml-3">{item.item_name} <span className="text-[var(--d-text-muted)] mr-1">x{item.quantity}</span></span>
              <span className="tabular-nums text-[var(--d-text)] shrink-0">₪{(item.item_price * item.quantity).toFixed(2)}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className={`px-4 py-2.5 flex items-center justify-between ${isDead ? "opacity-50" : ""}`}>
        <span className="text-[12px] font-bold text-[var(--d-text)]">الإجمالي</span>
        <div>

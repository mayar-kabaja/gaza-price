...
export default function AdminSuggestionsPage() {
  ...
  function load(showSpinner = true) {
    ...
    let url = `/api/gp-ctrl/products/pending?limit=${PAGE_SIZE}&offset=${offset}`;
  ...
  useEffect(() => {
    if (searchParams.get("add") === "1") {
      ...
      window.history.replaceState({}, "", "/gp-ctrl/suggestions");
    }
  }, [searchParams]);
  ...
  async function handleReview(id: string, action: "approve" | "reject") {
    ...
    try {
      const res = await fetch(`/api/gp-ctrl/products/${id}/review`, {
        method: "PATCH",
        headers: {
  ...
  async function confirmAddSubmit() {
    ...
    setAddSaving(true);
    try {
      const res = await fetch("/api/gp-ctrl/products", {
        method: "POST",

...
export default function AdminCategoriesPage() {
  ...
  async function confirmFormSubmit() {
    ...
    try {
      if (formMode === "add") {
        const res = await fetch("/api/gp-ctrl/categories", {
          method: "POST",
        ...
        const res = await fetch(`/api/gp-ctrl/categories/${editingCategory.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
          body: JSON.stringify(payload),
        });
  ...
  async function confirmDelete() {
    ...
    try {
      const res = await fetch(`/api/gp-ctrl/categories/${deleteTarget.id}`, {
        method: "DELETE",

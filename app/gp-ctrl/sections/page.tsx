...
export default function AdminSectionsPage() {
  ...
  async function confirmFormSubmit() {
    ...
    try {
      if (formMode === "add") {
        const res = await fetch("/api/gp-ctrl/sections", {
          method: "POST",
        ...
        const res = await fetch(`/api/gp-ctrl/sections/${editingSection.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
          body: JSON.stringify(payload),
        });
  ...
  async function confirmDelete() {
    ...
    try {
      const res = await fetch(`/api/gp-ctrl/sections/${deleteTarget.id}`, {
        method: "DELETE",

...
export default function AdminPlacesPage() {
  ...
  async function handleAddPlace() {
    ...
    setAddSaving(true);
    try {
      const res = await apiFetchAdmin("/api/gp-ctrl/places", {
        method: "POST",
        body: JSON.stringify({
          name: addName.trim(),
          section: addSection,
  ...
  async function handleSaveEdit() {
    ...
    try {
      ...
      const res = await apiFetchAdmin(`/api/gp-ctrl/places/${editPlace.id}`, {
        method: "PATCH",
  ...
  async function handleAction(id: string, action: "approve" | "suspend" | "pending") {
    ...
    try {
      let res;
      if (action === "pending") {
        res = await apiFetchAdmin(`/api/gp-ctrl/places/${id}`, {
          method: "PATCH",
          body: JSON.stringify({ status: "pending" }),
        });
      } else {
        res = await apiFetchAdmin(`/api/gp-ctrl/places/${id}/${action}`, { method: "PATCH" });
      }
      if (!res.ok) throw new Error("Failed");
      toast(labels[action]);
  ...
  async function handleToggleOpen(p: typeof places[0]) {
    ...
    try {
      const res = await apiFetchAdmin(`/api/gp-ctrl/places/${p.id}`, {
        method: "PATCH",
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await apiFetchAdmin(`/api/gp-ctrl/places/${deleteId}/suspend`, { method: "PATCH" });
      if (!res.ok) throw new Error("Failed");

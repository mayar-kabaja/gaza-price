...
const ADMIN_LOGIN = "/gp-ctrl/login";
...
function useAdminAuth() {
  ...
  const checkAuth = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/gp-ctrl/me", {
  ...
  const fetchCounts = useCallback(async (token: string) => {
    ...
    try {
      const [
        ...
        fetch("/api/gp-ctrl/products/pending?limit=1&offset=0", { headers }),
        fetch("/api/gp-ctrl/flags?limit=1&offset=0", { headers }),
        ...
        fetch("/api/gp-ctrl/contributors?limit=1&offset=0", { headers }),
        fetch("/api/reports?limit=1&offset=0", { headers: { Accept: "application/json" } }),
        fetch("/api/gp-ctrl/logs/search?limit=1&offset=0", { headers }),
        fetch("/api/gp-ctrl/logs/snapshots?limit=1&offset=0", { headers }),
        fetch("/api/gp-ctrl/places?status=pending&limit=1&offset=0", { headers }),
        fetch("/api/gp-ctrl/listings/pending?limit=1&offset=0", { headers }),
      ]);

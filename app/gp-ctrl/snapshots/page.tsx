...
export default function AdminSnapshotsPage() {
  ...
  function loadSnapshots() {
    ...
    fetch(`/api/gp-ctrl/logs/snapshots?${params}`, {
  ...
  async function handleRunSnapshotsJob() {
    ...
    setRunningJob(true);
    try {
      const res = await fetch("/api/gp-ctrl/jobs/run-snapshots", {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
          ...
          {loading ? (
            ...
            <div className="flex-1 min-h-0 overflow-auto">
              <table className="w-full min-w-[640px]">
                ...
                <tbody>
                  {snapshots.length === 0 ? (
                    <tr><td colSpan={8} className="py-12 text-center text-sm text-[#4E6070]">No price snapshots. Use &quot;Run snapshots now&quot; or wait for the daily job at midnight.</td></tr>
                  ) : snapshots.map((s, i) => (

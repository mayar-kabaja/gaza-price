...
export default function AdminListingsPage() {
  ...
  const fetchListings = useCallback(async (off = 0) => {
    ...
    try {
      const res = await fetch(`/api/gp-ctrl/listings?${params}`, {
  ...
  async function handleApprove(id: string) {
    ...
    const token = getAdminToken();
    try {
      const res = await fetch(`/api/gp-ctrl/listings/${id}/approve`, {
        method: "PATCH",
  ...
  async function handleReject(id: string) {
    ...
    const token = getAdminToken();
    try {
      const res = await fetch(`/api/gp-ctrl/listings/${id}/reject`, {
        method: "PATCH",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) throw new Error();
  ...
  async function handleStatusChange(id: string, status: string) {
    ...
    const token = getAdminToken();
    try {
      const res = await fetch(`/api/gp-ctrl/listings/${id}`, {
        method: "PATCH",
  ...
  async function confirmEdit() {
    ...
    try {
      ...
      const res = await fetch(`/api/gp-ctrl/listings/${editTarget.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({
  ...
  useEffect(() => {
    ...
    sellerDebounceRef.current = setTimeout(async () => {
      ...
      try {
        const token = getAdminToken();
        const res = await fetch(`/api/gp-ctrl/contributors?limit=6&offset=0&search=${encodeURIComponent(sellerSearch.trim())}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  async function confirmAdd() {
    ...
    if (!sellerId) {
      try {
        const meRes = await fetch("/api/gp-ctrl/me", { headers: { Authorization: `Bearer ${token}` } });
    ...
    try {
      ...
      const res = await fetch("/api/gp-ctrl/listings", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({
          seller_id: sellerId,
  ...
  async function confirmDelete() {
    ...
    const token = getAdminToken();
    try {
      const res = await fetch(`/api/gp-ctrl/listings/${deleteTarget.id}/reject`, {
        method: "PATCH",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) throw new Error();

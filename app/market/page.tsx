  ...
  if (isDesktop) {
    return (
      <div className="p-5 h-full overflow-y-auto">
        ...
        {isLoading && <div className="space-y-3">{Array.from({ length: 6 }).map((_, i) => <ListingCardSkeleton key={i} />)}</div>}
        ...
        {!isLoading && allListings.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center bg-surface rounded-2xl border border-border">
            ...
            <Link href="/market/new" className="px-5 py-2 bg-olive text-white rounded-full text-sm font-semibold">+ أضف إعلاناً</Link>
          </div>
        )}
      ...
      <div className="flex-1 overflow-y-auto pb-24">
        ...
        {!isLoading && allListings.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
            ...
            <Link href="/market/new" className="px-5 py-2.5 bg-olive text-white rounded-full font-semibold text-sm">
              + أضف إعلاناً
            </Link>
          </div>
        )}

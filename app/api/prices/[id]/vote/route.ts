...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json({ error: "BAD_REQUEST", message: "Missing vote_type" }, { status: 400 });
  }

  const url = `${base}/prices/${priceId}/vote`;
  try {
    ...
    const rawMessage = err instanceof Error ? err.message : "خطأ في التصويت";

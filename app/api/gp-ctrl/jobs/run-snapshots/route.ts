...
/** POST /api/gp-ctrl/jobs/run-snapshots — Trigger daily price snapshots job. Requires admin JWT. */
export async function POST(req: NextRequest) {
  ...
  try {
    const res = await fetch(`${base}/gp-ctrl/jobs/run-snapshots`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        Authorization: `Bearer ${token}`,
      },
      signal: AbortSignal.timeout(60000), // may take a while
    });

...
/** GET /api/gp-ctrl/reports — Admin reports feed (includes all statuses). Requires admin JWT. */
export async function GET(req: NextRequest) {
  ...
  try {
    const params = new URLSearchParams({ filter, limit, offset });
    if (search) params.set("search", search);
    const res = await fetch(`${base}/gp-ctrl/reports?${params}`, {
